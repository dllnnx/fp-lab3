...
let parse_args () =
  ...
  let rec parse_algorithms algorithms step args =
    match args with
    | "--linear" :: rest -> 
      parse_algorithms (Linear :: algorithms) step rest
    | "--lagrange" :: n :: rest -> 
      (try parse_algorithms ((Lagrange (int_of_string n)) :: algorithms) step rest
    ...
    | [] -> (algorithms, step)
  in
  let (algs, step) = parse_algorithms [] 0.1 (List.tl args) in  (* skip program name *)
  (List.rev algs, step)
...
let process_stream algorithms step =
  let window_size = 
    List.fold_left max 2 (List.map (function Linear -> 2 | Lagrange n -> n) algorithms) in
  
  let rec loop window =
    match Io.read_point_stream () with (* one point at a time *)
    | Some point ->
      flush stdout;
      ...
      if List.length new_window >= window_size then
        ...
        List.iter (fun algorithm ->
          let interpolated = interpolate_stream algorithm step reversed_window in
...
let main () =
  ...
  if List.length algorithms > 0 then
    process_stream algorithms step
  else
