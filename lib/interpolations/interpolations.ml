  ...
  | _ ->
    let rec generate_points_with_step (x1, y1) (x2, y2) acc =
      if x1 +. step >= x2 then acc
        ...
        generate_points_with_step (x1 +. step, y) (x2, y2) ((x1 +. step, y) :: acc)
    in
    let rec interpolate acc pts =
      match pts with
      | (x1, y1) :: (x2, y2) :: rest ->
        let intermediate_points = generate_points_with_step (x1, y1) (x2, y2) acc in
...
let lagrange_interpolation_step step n points =
  if List.length points < n then []
    ...
    let window = 
      let rec take k lst =
        if k <= 0 then [] else match lst with x :: xs -> x :: take (k - 1) xs | [] -> []
      in take n points in
