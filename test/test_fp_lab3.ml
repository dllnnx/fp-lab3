...
module TestInterpolations = struct
  ...
  let test_linear_interpolation_simple () =
    ...
    Alcotest.(check (list testable_point))
      "simple linear interpolation" expected result
  ...
  let test_linear_interpolation_multiple_segments () =
    ...
    Alcotest.(check (list testable_point))
      "multiple segments linear interpolation" expected result
...
module TestIO = struct
  ...
  let test_parse_valid_point () =
    ...
    Alcotest.(check (option testable_point))
      "valid point parsing" expected result

  let test_parse_invalid_point () =
    let result = Io.parse_point "invalid input" in
    let expected = None in
    Alcotest.(check (option testable_point))
      "invalid point parsing" expected result

  let test_parse_point_with_extra_spaces () =
    let result = Io.parse_point "  1.5   2.3  " in
    let expected = None in
    Alcotest.(check (option testable_point))
      "point with extra spaces" expected result
...
let () =
  Alcotest.run "fp-lab3"
    [
      ( "interpolations",
        [
          Alcotest.test_case "Linear interpolation simple" `Quick
            TestInterpolations.test_linear_interpolation_simple;
          Alcotest.test_case "Linear interpolation multiple segments" `Quick
            TestInterpolations.test_linear_interpolation_multiple_segments;
          Alcotest.test_case "Linear interpolation empty list" `Quick
            TestInterpolations.test_linear_interpolation_empty_list;
          Alcotest.test_case "Linear interpolation single point" `Quick
            TestInterpolations.test_linear_interpolation_single_point;
          Alcotest.test_case "Lagrange interpolation step simple" `Quick
            TestInterpolations.test_lagrange_interpolation_step_simple;
          Alcotest.test_case "Lagrange interpolation insufficient points" `Quick
            TestInterpolations.test_lagrange_interpolation_insufficient_points;
        ] );
      ( "io",
        [
          Alcotest.test_case "Parse valid point" `Quick
            TestIO.test_parse_valid_point;
          Alcotest.test_case "Parse invalid point" `Quick
            TestIO.test_parse_invalid_point;
          Alcotest.test_case "Parse point with extra spaces" `Quick
            TestIO.test_parse_point_with_extra_spaces;
